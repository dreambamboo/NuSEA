...
class Option(object):
    def __init__(self):
        ...
        self.is_assist = True
        self.seed = 3407
        self.input_size = 80 # resize
        self.batchsize = 256
        self.model_dir =  '../Models_with_Parameters'
        self.pretrained = True 
        self.pretrain_net = os.path.join(self.model_dir,'CPM17.pth')#You may get this file at https://mcprl.com/html/dataset/NuSEA.html
        ...
        self.env = 'cell-miccai2017'

        self.net_select = 'unet-dcn'
        
        self.early_stop = 100
        self.epochs = 100
        self.check_epoch = 5#self.epochs
        self.model_tag = 'test-'+self.net_select

        self.curve_tag = self.model_tag

        # test
        self.testset_crop_images = 'miccai-segmentation-2017-test.dat' #You may get this file at https://mcprl.com/html/dataset/NuSEA.html
        self.testset_truth = 'miccai-segmentation-2017-test-truth.dat' #You may get this file at https://mcprl.com/html/dataset/NuSEA.html
...
class Cell_Segmentation(object):
    ...
    def _test_(self):
        ...
        for class_dir in ['gbm', 'hnsc', 'lgg', 'lung']:
            print(class_dir,"*"*10)
            for img_index in range(8):
                img_name = "image0"+str(img_index+1)
                ...
                for cell in testset_dict[class_dir][img_name]:
                    ...
                    cv2.ellipse(label3, (np.int32(cx), np.int32(cy)),(np.int32(a/2), np.int32(b/2)), angle, 0, 360, (255,0,0),-1) # 填充255的椭圆
                    ellipse = label3[:,:,0]
                    ...
                    if batch_img.shape[0] == self.params.batchsize:
                        ...
                        batch_img = np.zeros((1,self.params.input_size,self.params.input_size,3)) # new batch
                        batch_ellipse = np.zeros((1,self.params.input_size,self.params.input_size)) # new batch
                    else:
                        continue
                ...
                truth_h, truth_w, _ = truth.shape
                predict = np.zeros((truth_h, truth_w, 10)) 
                ...
                for cell_index, single_cell in enumerate(testset_dict[class_dir][img_name]):
                    ...
                    if self.params.is_assist_test:
                        # cell_predict_ellipse_mask = cell_ellipse*255
                        cell_predict_ellipse_mask = cell_ellipse*cell_predict_resize 
                    else:
                        cell_predict_ellipse_mask = cell_predict_resize 
                    ...
                    position_w, position_h = single_cell["position_w_h"] 
